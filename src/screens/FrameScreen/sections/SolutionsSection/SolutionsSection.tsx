      ...
      <div className="relative w-full max-w-7xl mx-auto px-4">
        ...
        <div className="relative z-[5]">
          <div className="flex flex-col items-center">
            <h2 className={`${currentTheme.text} text-center font-bold text-xl md:text-[44px] mb-8 md:mb-12 font-['Poppins',Helvetica]`}>
                ...
                >
                  {faqItems.map((item, index) => (
                    <AccordionItem
                      key={`item-${index}`}
                      value={`item-${index}`}
                      className={`rounded-[10px] border border-solid px-4 md:px-6 py-3 md:py-4 ${
                        theme === 'dark' 
                          ? 'border-[#2a2a2a] bg-[linear-gradient(137deg,rgba(28,28,28,1)_0%,rgba(5,5,5,1)_100%)]'
                          : 'border-gray-200 bg-white shadow-lg'
                      }`}
                    >
                      <AccordionTrigger className={`font-['Poppins',Helvetica] font-bold text-sm md:text-lg leading-tight md:leading-[21.6px] hover:no-underline text-left ${currentTheme.text}`}>
                        {item.question}
                      </AccordionTrigger>
                      <AccordionContent className={`font-['Poppins',Helvetica] font-normal text-sm md:text-base leading-relaxed md:leading-6 pt-2 ${theme === 'dark' ? 'text-white' : 'text-gray-600'}`}>
                        {item.answer}
                      </AccordionContent>
                    </AccordionItem>
                  ))}
