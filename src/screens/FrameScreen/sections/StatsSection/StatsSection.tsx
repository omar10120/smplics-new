...
export const StatsSection = (): JSX.Element => {
  ...
  // Navigation links
  const navLinks = [
    { name: "Home", active: true, href: "#home" },
  ...
  return (
    <section id="home" className="w-full">
      {/* Top contact bar */}
      <header className={`w-full ${theme === 'dark' ? 'bg-[#1f1e1b]' : 'bg-gray-50'} py-3 md:py-5`}>
          ...
          <div className="flex items-center gap-3 md:gap-6 text-xs md:text-sm">
            {contactInfo.map((item, index) => (
              <div key={`contact-${index}`} className="flex items-center gap-2">
                ...
                <span className={`font-normal text-[13.1px] leading-[21px] ${theme === 'dark' ? 'text-white' : 'text-gray-900'} font-['Poppins',Helvetica]`}>
                  {item.text}
                </span>
              </div>
            ))}
      ...
      {/* Main navigation */}
      <nav className={`w-full ${theme === 'dark' ? 'bg-[#1f1e1b]' : 'bg-gray-50'} py-2 md:py-4 sticky top-0 z-50`}>
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-center gap-4 md:gap-0 px-4">
              ...
              >
                <span
                  className={`font-normal text-sm md:text-base text-center leading-[22px] font-['Poppins',Helvetica] ${
                    link.active ? "text-[#ee7639]" : theme === 'dark' ? "text-white" : "text-gray-900"
      ...
      {/* Hero section */}
      <div className={`w-full ${theme === 'dark' ? 'bg-[#1f1e1b]' : 'bg-white'} overflow-hidden`}>
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-8 md:gap-12 py-8 md:py-16">
              <div className="flex flex-col gap-4 md:gap-[18px] max-w-full md:max-w-[650px]">
                <div className="flex items-center">
                  <h1 className={`${theme === 'dark' ? 'text-white' : 'text-gray-900'} text-2xl md:text-[44px] font-bold leading-tight md:leading-[54px] font-['Poppins',Helvetica]`}>
                ...
                <p className={`${theme === 'dark' ? 'text-white' : 'text-gray-700'} text-sm md:text-base font-normal leading-relaxed md:leading-[26.9px] font-['Poppins',Helvetica]`}>
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3 md:gap-4 mt-2">
                  ...
                  <Button
                    onClick={() => handleNavClick('#contact')}
                    variant="outline"
                    className="border-[#ee7639] text-f-5f-5f-4 rounded-[80px] shadow-[0px_22px_44px_#ee763966] h-10 md:h-12 px-4 w-full sm:w-auto"
                  >
                    <span className="font-semibold text-xs md:text-sm text-ee-7639">Let&apos;s Talk</span>
                  </Button>
                </div>
              </div>

              <div className="flex-shrink-0 relative">
                <div className="absolute -top-4 -left-4 md:-top-6 md:-left-6 w-16 h-8 md:w-[110px] md:h-[54px] bg-yellow-50 rounded-md shadow-card-shadow-1">
        ...
        <div id="about" className="container mx-auto py-8 md:py-16 px-4">
          <div className="flex flex-col items-center gap-8 md:gap-[69px] max-w-full md:max-w-[967px] mx-auto">
            <div className="flex flex-col items-center gap-4 md:gap-[18px] text-center">
              ...
              <div className="flex items-center">
                <h2 className={`${theme === 'dark' ? 'text-white' : 'text-gray-900'} text-2xl md:text-[44px] font-bold leading-tight md:leading-[22px] font-['Poppins',Helvetica]`}>
              ...
              <p className={`${theme === 'dark' ? 'text-white' : 'text-gray-700'} text-sm md:text-base font-normal leading-relaxed md:leading-[26.9px] text-center max-w-full md:max-w-[762px] font-['Poppins',Helvetica]`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-[60px] w-full">
              {statsCards.map((card, index) => (
                <Card
                  key={`stat-${index}`}
                  className={`rounded-[7.35px] border-[0.74px] border-solid ${theme === 'dark' ? 'border-[#2a2a2a] bg-[linear-gradient(137deg,rgba(28,28,28,1)_0%,rgba(5,5,5,1)_100%)]' : 'border-gray-200 bg-white shadow-lg'}`}
                >
                  <CardContent className="flex flex-col items-center justify-center p-4 md:p-8">
                    <span className="font-display-8-semi-bold text-ee-7639 mt-[-0.74px] text-lg md:text-2xl">
                      {card.value}
                    </span>
                    <span className={`font-['Poppins',Helvetica] font-semibold ${theme === 'dark' ? 'text-white' : 'text-gray-900'} text-xs md:text-base text-center leading-tight md:leading-[22px]`}>
                      {card.label}
                    </span>
                  </CardContent>
                </Card>
